...
class Memory() {
    ...
    fun modifyMemory(operation:Int,modifyValue:Int,absoluteAddress:Int, nibbleLength:Int){
        ...
        //TODO:判斷正負
        val intToBeModified = if(stringToBeModified.hexIsNegative()) ((stringToBeModified.hexTwosComplement().toDecimalInt())*-1) else stringToBeModified.toDecimalInt()
...
class ByteString(){
    ...
    fun setByte(byteHexString:String){
        higherNibble = if(byteHexString[0]=='*'){ "*" }else{ byteHexString[0].uppercase() }
        lowerNibble = if(byteHexString[1]=='*'){ "*" }else{ byteHexString[1].uppercase() }
    }
